...
namespace SignIn.DL
{
    internal class cartDL
    {
        ...
        public static void storeCustomOrderInFile(ref string custNameForFile, customerBL cust) // customer after paying bill
        {
           ...
           // DateTime timeNow = DateTime.Today.Date;
            ...
            if (File.Exists(path))
            {
                ...
                while ((record = fileOrder.ReadLine()) != null)
                {
                    ...
                    int custOrder = int.Parse(record.Split(';')[3]);
                    int itemAmount = int.Parse(record.Split(';')[4]);
                    DateTime date = DateTime.Parse(record.Split(';')[5]);
                        ...
                        if (i.NameofMainItem == mainItem)
                        {
                            foreach (var j in i.SubProduct)
                            {
                                if (j.PName == Pname)
                                {
                                    ...
                                    {*/
                                    customOrder obj = new customOrder(custName, mainItem, Pname, custOrder, itemAmount,date);
        ...
        {
            string name = customOrderList[position].CustNameOrder;
            return name;
        }
        ...
        {
            string name = customOrderList[position].OrderName;
            return name;
        }
        public static DateTime findCustomOrderDilivDate(int position) // find the product name which is added by the customer in his order
        {
            ...
            foreach (var i in cartDL.customOrderList)
            {
                if (i is customOrder j)
                {
                    string name = cartDL.findCustomOrderProductName(position);
                    string Cname =cartDL.findCustomOrderCustName(position);
                    if (j.OrderName == name && j.CustNameOrder == Cname)
                    {
